...
#         @table "obj", [ ["",""],["",""] ]
        ...
        <ul><li> La non­linéarité des problèmes qui rend les calculs couteux et met à défaut bon nombre de méthodes.</li>
        <li>La non­localité des perturbations dûes aux phénomènes propagatifs qui sont donc plus difficiles à capter avec un modèle réduit.</li></ul>
        En plus de l'intégration de ces nouvelles méthodes dans un outil à portée industrielle, ce projet vise aussi à fournir :
        <ul><li>Des critères de qualité relatifs à un modèle réduit (prédictivité, robustesse, représentativité, domaine d'application, maîtrise des incertitudes)</li>
        Des méthodes permettant de choisir une famille de modèles réduits répondant à un compromis entre le poids d'un modèle (finesse) et le nombre de modèles (valides 
...
#         @gallery "fin", [ essone_logo, bpi_logo, idf_logo, systematic_logo ]
        ...
        @table "new", [
            ...
            L'équipe Scilab et ses partenaires vous présenteront le 28 et 29 Juin au Ter@tec 2016 sur le stand Systematic les avancées du projet.<br>&nbsp" ],
